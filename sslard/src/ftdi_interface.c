...
int load_ftdi_device_image_data( _FTDI_DEVICE* ftdi_device, char* image_data, int image_size )
{
	...
	ftdi_ret = ftdi_write_data( &ftdi_device->context, cmd_buffer, 1 );
...
{		
	...
	// set the flag to update the registers on first-boot
...
int update_device_registers( _FTDI_DEVICE* ftdi_device, int defaults )
{
	...
	if( !ftdi_device )
	{
		daemon_log( LOG_INFO, "%s : %s : failed to update device registers, ftdi_device pointer is null", FTDI_LOG_PREFIX, LOG_ERROR );
		return FTDI_FAIL;
	}
#endif
